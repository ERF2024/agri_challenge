...
namespace Unity.Robotics.UrdfImporter.Control
{
    public class RobotControl : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            //ros.Subscribe<JointStateMsg>(topicNameCmd, Joi);
        ...
        public void MimicJointStates(RosMessageTypes.Sensor.JointStateMsg response)
        {
            ...
            double[] joint_states = jointPositions.Select(r => r * Mathf.Rad2Deg).ToArray();

            var jointVelocities = response.velocity;
            double[] resultVel = jointVelocities.Select(r => r * Mathf.Rad2Deg).ToArray();

            double[] result = {joint_states[5], joint_states[0], joint_states[1], joint_states[2], joint_states[3], joint_states[4]};

            //muovo 1 e si muove 0
            //double temp = result[1];
            //result[1] = result[0];
            //result[0] = temp;
            //result[1] += -90.0;
            //movo lo 0 e si muove 4
            ...
            foreach (ArticulationBody joint in articulationChain)
            {
                ...
                joint1XDrive.stiffness = 10000;
                joint1XDrive.damping = 100;
                joint1XDrive.target = (float)result[idx];
                joint1XDrive.targetVelocity = (float)resultVel[idx];
                joint.xDrive = joint1XDrive;
